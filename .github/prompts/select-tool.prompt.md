...
This prompt mirrors the `select-tool` skill — see `.github/skills/select-tool/SKILL.md` for the full behavioral spec. Exists only so VS Code and JetBrains get the same explicit `/select-tool` invocation Copilot CLI gets natively from Skills.
