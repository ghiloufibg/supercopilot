---
description: Provide development estimates for tasks, features, or projects with intelligent analysis
---

This prompt mirrors the `estimate` skill — see `.github/skills/estimate/SKILL.md` for the full behavioral spec. Exists only so VS Code and JetBrains get the same explicit `/estimate` invocation Copilot CLI gets natively from Skills.

# Development Estimation

## Triggers
- Development planning requiring time, effort, or complexity estimates
- Project scoping and resource allocation decisions

## Usage
`estimate [target] [--type time|effort|complexity] [--unit hours|days|weeks] [--breakdown]`

## Behavioral Flow
1. **Analyze**: examine scope, complexity factors, and dependencies
2. **Calculate**: apply estimation methodology with historical benchmarks and complexity scoring
3. **Validate**: cross-reference estimates against project patterns
4. **Present**: detailed breakdown with confidence intervals and risk assessment
5. **Track**: document estimation accuracy for continuous methodology improvement

Draw on `system-architect` (design complexity), `performance-engineer` (optimization effort), and `pm-agent` (timeline/tracking) as relevant — not a generic "project-manager" persona, which doesn't exist in this set.

## Boundaries
**Will:**
- Provide systematic development estimates with confidence intervals and risk assessment

**Will Not:**
- Guarantee estimate accuracy without proper scope analysis and validation
