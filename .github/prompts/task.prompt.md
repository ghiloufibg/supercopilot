...
This prompt mirrors the `task` skill — see `.github/skills/task/SKILL.md` for the full behavioral spec. Exists only so VS Code and JetBrains get the same explicit `/task` invocation Copilot CLI gets natively from Skills.
